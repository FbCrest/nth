...
import {
  initPlayer, subscribe, togglePlay, nextTrack, prevTrack,
  setVolume, toggleMute, seekTo, playVideoAt, getPlaylistLength,
  type PlayerState,
} from '../lib/ytPlayer';

const EDGE_GAP = 20;
const PLAYER_ELEMENT_ID = 'nth-yt-player-iframe';
...
export default function MusicPlayer() {
  ...
  const [st, setSt] = useState<PlayerState>({
    playing: false, title: 'Đang tải...', artist: 'OST Nghịch Thủy Hàn',
    thumbnail: '', currentTime: 0, duration: 0,
  ...
  // Init player một lần
  useEffect(() => {
    initPlayer(PLAYER_ELEMENT_ID);
    const unsub = subscribe(setSt);
    return unsub;
  }, []);
  ...
  const fillPct    = st.duration > 0 ? (st.currentTime / st.duration) * 100 : 0;
  const volFillPct = st.muted ? 0 : st.volume;
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        border: `1px solid rgba(192,57,43,${st.playing ? '0.55' : '0.25'})`,
        clipPath: 'polygon(7px 0, 100% 0, 100% calc(100% - 7px), calc(100% - 7px) 100%, 0 100%, 0 7px)',
        overflow: 'hidden', position: 'relative',
        transition: 'border-color 300ms',
        background: 'rgba(192,57,43,0.06)',
        boxShadow: st.playing ? `0 0 10px rgba(192,57,43,0.2)` : 'none',
      }}>
        {st.thumbnail
          ? <img src={st.thumbnail} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', opacity: st.playing ? 1 : 0.6 }} />
        ...
        {st.playing && (
          <div style={{ position: 'absolute', bottom: 5, left: 0, right: 0, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: 2, padding: '0 7px' }}>
            {vizBars.map((h, i) => (
              <div key={i} style={{ width: 3, height: `${Math.round(h * 18)}px`, background: i === 2 ? '#fff' : 'rgba(255,255,255,0.7)', borderRadius: 1, transition: 'height 80ms ease', alignSelf: 'flex-end', boxShadow: '0 0 4px rgba(0,0,0,0.8)' }} />
            ))}
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 5 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <span style={{ fontSize: 9, color: C.textSub, minWidth: 22, fontVariantNumeric: 'tabular-nums' }}>{fmt(st.currentTime)}</span>
          <div style={{ flex: 1, position: 'relative', height: 20, display: 'flex', alignItems: 'center' }}>
            ...
            <input type="range" min={0} max={st.duration || 100} step={0.5} value={st.currentTime}
              onChange={e => seekTo(Number(e.target.value))}
              style={{ position: 'absolute', left: 0, right: 0, width: '100%', opacity: 0, cursor: 'pointer', height: 20, margin: 0, padding: 0 }}
            />
          </div>
          <span style={{ fontSize: 9, color: C.textSub, minWidth: 22, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmt(st.duration)}</span>
        </div>

        {/* Controls */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <SmBtn onClick={() => setShuffle(s => !s)} title="Ngẫu nhiên" active={shuffle}>
          ...
          <MidBtn onClick={prevTrack} title="Bài trước">
            <svg viewBox="0 0 24 24" width={15} height={15}><path fill="currentColor" d="M6 6h2v12H6zm3.5 6 8.5 6V6z" /></svg>
          </MidBtn>

          {/* Play/Pause */}
          <button onClick={togglePlay} title={st.playing ? 'Dừng' : 'Phát'}
            style={{ width: 38, height: 38, display: 'flex', alignItems: 'center', justifyContent: 'center', background: st.ready ? C.red : 'rgba(192,57,43,0.2)', border: 'none', clipPath: 'polygon(6px 0, 100% 0, 100% calc(100% - 6px), calc(100% - 6px) 100%, 0 100%, 0 6px)', cursor: st.ready ? 'pointer' : 'default', color: '#fff', boxShadow: st.ready ? `0 0 14px rgba(192,57,43,0.45)` : 'none', transition: 'background 180ms' }}
            onMouseEnter={e => st.ready && (e.currentTarget.style.background = C.redLight)}
            onMouseLeave={e => (e.currentTarget.style.background = st.ready ? C.red : 'rgba(192,57,43,0.2)')}
          >
            {!st.ready
          ...
          <MidBtn onClick={() => {
            if (shuffle) {
              const len = getPlaylistLength();
              if (len > 0) playVideoAt(Math.floor(Math.random() * len));
            } else {
              nextTrack();
            }
          }} title="Bài tiếp">
          ...
          <SmBtn onClick={() => setRepeat(r => !r)} title="Lặp lại" active={repeat}>
        ...
        {/* Volume */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <button onClick={toggleMute} title="Tắt/Bật tiếng"
          ...
          <div style={{ flex: 1, position: 'relative', height: 20, display: 'flex', alignItems: 'center' }}>
            <div style={{ position: 'absolute', left: 0, right: 0, height: 3, background: C.track, pointerEvents: 'none' }}>
              <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${volFillPct}%`, background: C.red }} />
            </div>
            <input type="range" min={0} max={100} step={1} value={st.muted ? 0 : st.volume}
              onChange={e => setVolume(Number(e.target.value))}
              style={{ position: 'absolute', left: 0, right: 0, width: '100%', opacity: 0, cursor: 'pointer', height: 20, margin: 0, padding: 0 }}
            />
          </div>
          <span style={{ fontSize: 9, color: C.textSub, minWidth: 18, textAlign: 'right' }}>{st.muted ? 0 : st.volume}</span>
        </div>
      </div>
    </div>
  );
