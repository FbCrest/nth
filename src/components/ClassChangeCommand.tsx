...
export default function ClassChangeCommand() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="p-4 sm:p-6 max-w-4xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-black tracking-tight uppercase" style={{ color: 'var(--text-1)' }}>Câu lệnh đổi phái</h2>
        <div className="mt-1.5 h-0.5 w-10 bg-linear-to-r from-amber-500 to-orange-500 rounded-full" />
      </div>
          ...
          {classes.map((c, i) => (
            ...
            >
              ...
              <div className="text-center">
                ...
                <div className="font-chinese text-[16px] leading-tight mt-0.5 select-text" style={{ color: 'var(--text-3)' }}>{c.zh}</div>
              </div>
            </motion.div>
          ))}
        </div>
