...
import { quanCoRoles as initialRoles } from '../data/co-nghich-roles';
import { lienKetPheData } from '../data/co-nghich-lien-ket-phe';
import { lienKetPhaiData } from '../data/co-nghich-lien-ket-phai';
...
// Lien ket phe/phai - from data files
const LIEN_KET_PHE = lienKetPheData.map(p => ({ ten: p.ten, slug: p.slug }));
const LIEN_KET_PHAI = lienKetPhaiData.map(p => ({ ten: p.ten, slug: p.slug }));

// ── Màu theo loại quân cờ — lấy từ roles data ──
function hexToRoleStyle(color: string) {
  return { color, bg: color + '1a', border: color + '59' };
}
function getLoaiStyle(loai: string) {
  const role = initialRoles.find(r => r.label === loai);
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, minHeight: 26 }}>
              <div style={{ flexShrink: 0 }}>
                {item.loai && (() => { const s = getLoaiStyle(item.loai); return (
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 700, padding: '2px 10px', borderRadius: 4, background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>
                    {item.loai}
                  </span>
                ); })()}
              </div>
            ...
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 8, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              {item.lien_ket_phe && item.lien_ket_phe.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', flexShrink: 0 }}>Liên kết thế lực:</span>
                  {item.lien_ket_phe.map(slug => {
                    const info = lienKetPheData.find(p => p.slug === slug);
                    if (!info) return null;
                    return (
                      <div key={slug} title={info.ten} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px 2px 4px', borderRadius: 20, background: 'var(--bg-sunken)', border: '1px solid var(--border)' }}>
                        <img src={info.image_url ?? `/images/co-nghich-thuy-han/lien-ket-phe/${slug}.png`} alt={info.ten} style={{ width: 32, height: 32, objectFit: 'contain' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
              ...
              {item.lien_ket_phai && item.lien_ket_phai.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', flexShrink: 0 }}>Liên kết chức nghiệp:</span>
                  {item.lien_ket_phai.map(slug => {
                    const info = lienKetPhaiData.find(p => p.slug === slug);
                    if (!info) return null;
                    return (
                      <div key={slug} title={info.ten} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px 2px 4px', borderRadius: 20, background: 'var(--bg-sunken)', border: '1px solid var(--border)' }}>
                        <img src={info.image_url ?? `/images/co-nghich-thuy-han/lien-ket-phai/${slug}.png`} alt={info.ten} style={{ width: 32, height: 32, objectFit: 'contain' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                        <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-2)', whiteSpace: 'nowrap' }}>{info.ten}</span>
                      </div>
                    );
                  })}
...
// ── Modal Chi Tiết Liên Kết (Thế Lực / Chức Nghiệp) ──
type LienKetModalItem = {
...
function LienKetDetailModal({ item, onClose }: { item: LienKetModalItem; onClose: () => void }) {
  // Parse hieu_ung_lien_ket: mỗi dòng dạng "2 Nội dung hiệu ứng"
  const parseHieuUng = (raw: string | null | undefined): { moc: string; noi_dung: string }[] => {
    if (!raw) return [];
    return raw.split('\n').filter(Boolean).map(line => {
      const m = line.match(/^(\d+)\s+([\s\S]+)$/);
      if (m) return { moc: m[1], noi_dung: m[2] };
      return { moc: '', noi_dung: line };
    });
  };

  const hieuUng = parseHieuUng(item.hieu_ung_lien_ket);
  const quanCoImages = item.quan_co_images ?? [];
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, y: 12 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 12 }}
        transition={{ duration: 0.2 }}
        className="border shadow-2xl w-full"
        style={{ maxWidth: 680, maxHeight: '90vh', overflowY: 'auto', backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)', borderRadius: 16 }}
        onClick={e => e.stopPropagation()}>

        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 20px', borderBottom: '1px solid var(--border)', background: 'var(--bg-sunken)' }}>
          <div style={{ width: 64, height: 64, flexShrink: 0, overflow: 'hidden', background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            {item.image_url
              ? <img src={item.image_url} alt={item.ten} style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }} />
              : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, opacity: 0.15 }}>⚔</div>
            }
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 20, fontWeight: 400, color: 'var(--text-1)', fontFamily: 'var(--font-skill, inherit)', lineHeight: 1.3, marginBottom: 2 }}>{item.ten}</div>
            {item.ten_zh && (
              <div style={{ fontSize: 13, color: 'var(--text-3)', fontFamily: 'var(--font-chinese, serif)', letterSpacing: '0.05em' }}>{item.ten_zh}</div>
            )}
          </div>
          <button onClick={onClose}
            style={{ flexShrink: 0, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', fontSize: 22, padding: '2px 4px', lineHeight: 1 }}
        ...
        <div style={{ padding: '14px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* Mô tả */}
          {item.phan_thuong && (
            <div style={{ fontSize: 13.5, color: 'var(--text-2)', lineHeight: 1.75, borderBottom: '1px solid var(--border)', paddingBottom: 14 }}>
              <HighlightText text={item.phan_thuong} />
            </div>
          )}

          {/* Hiệu ứng liên kết */}
          {hieuUng.length > 0 && (
            <div style={{ borderBottom: quanCoImages.length > 0 ? '1px solid var(--border)' : 'none', paddingBottom: quanCoImages.length > 0 ? 14 : 0 }}>
              <div style={{ fontSize: 12, fontWeight: 800, color: ACCENT, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 12, textAlign: 'center' }}>
                Hiệu Ứng Liên Kết
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {hieuUng.map((h, i) => (
                  <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ fontSize: 13.5, color: 'var(--text-1)', lineHeight: 1.7, flex: 1 }}>
                      <HighlightText text={h.noi_dung} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Danh sách quân cờ — không có title */}
          {quanCoImages.length > 0 && (
            <div style={{ borderRadius: 10, border: '1px solid var(--border)', overflow: 'hidden', padding: 10, backgroundColor: 'var(--bg-sunken)' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {quanCoImages.map((url, i) => (
                  <div key={i} style={{ width: 80, height: 100, flexShrink: 0, overflow: 'hidden', border: '1px solid var(--border)' }}>
          ...
          {/* Empty state */}
          {hieuUng.length === 0 && quanCoImages.length === 0 && (
            <div style={{ padding: '32px 0', textAlign: 'center' }}>
              <div style={{ fontSize: 32, opacity: 0.1, marginBottom: 8 }}>⚔</div>
              <div style={{ fontSize: 13, color: 'var(--text-3)' }}>Dữ liệu đang được cập nhật</div>
            </div>
          )}
...
// ── Tab Chú Thích Icon ──
function ChuThichTab() {
  const [selectedLienKet, setSelectedLienKet] = useState<LienKetModalItem | null>(null);

  return (
    <div style={{ padding: '24px 0' }}>
      {/* Chú thích icon */}
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-1)', marginBottom: 6 }}>Chú Thích Icon</h2>
        <p style={{ fontSize: 13, color: 'var(--text-3)', lineHeight: 1.6 }}>
          Các icon dưới đây xuất hiện trong mô tả kỹ năng quân cờ và buff kiếm để biểu thị chỉ số thuộc tính.
        </p>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12, marginBottom: 40 }}>
        {coNghichIcons.map(icon => (
          <div key={icon.slug} style={{
            display: 'flex', alignItems: 'center', gap: 14,
            padding: '14px 16px', borderRadius: 10,
            background: 'var(--bg-card)', border: '1px solid var(--border)',
          }}>
            <div style={{ width: 44, height: 44, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-sunken)', borderRadius: 8, padding: 4 }}>
              <img src={icon.image_url} alt={icon.label} style={{ width: 36, height: 36, objectFit: 'contain' }} />
            </div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-1)' }}>{icon.label}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginBottom: 32 }}>
        <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-1)', marginBottom: 6 }}>Liên Kết Thế Lực</h2>
        <p style={{ fontSize: 13, color: 'var(--text-3)', lineHeight: 1.6, marginBottom: 16 }}>
          Click vào một thế lực để xem hiệu ứng liên kết và danh sách quân cờ.
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 10 }}>
          {lienKetPheData.map(phe => (
            <button key={phe.slug} onClick={() => setSelectedLienKet(phe)}
              style={{
              ...
              onMouseEnter={e => {
                (e.currentTarget as HTMLElement).style.borderColor = 'rgba(212,160,60,0.5)';
                (e.currentTarget as HTMLElement).style.background = 'rgba(212,160,60,0.06)';
              }}
              onMouseLeave={e => {
              ...
              <div style={{ width: 40, height: 40, flexShrink: 0 }}>
                {phe.image_url
                  ? <img src={phe.image_url} alt={phe.ten} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                  : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, opacity: 0.2 }}>⚔</div>
                }
              </div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)' }}>{phe.ten}</div>
                {phe.phan_thuong && (
                  <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 140 }}>
                    {phe.phan_thuong}
                  </div>
                )}
              </div>
      ...
      {/* Liên kết Chức Nghiệp */}
      <div>
        <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-1)', marginBottom: 6 }}>Liên Kết Chức Nghiệp</h2>
        <p style={{ fontSize: 13, color: 'var(--text-3)', lineHeight: 1.6, marginBottom: 16 }}>
          Click vào một chức nghiệp để xem hiệu ứng liên kết và danh sách quân cờ.
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 10 }}>
          {lienKetPhaiData.map(phai => (
            <button key={phai.slug} onClick={() => setSelectedLienKet(phai)}
              style={{
                display: 'flex', alignItems: 'center', gap: 12,
                padding: '12px 14px', borderRadius: 10, cursor: 'pointer',
                background: 'var(--bg-card)', border: '1px solid var(--border)',
              ...
              <div style={{ width: 40, height: 40, flexShrink: 0 }}>
                {phai.image_url
                  ? <img src={phai.image_url} alt={phai.ten} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                  : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, opacity: 0.2 }}>⚔</div>
                }
              </div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)' }}>{phai.ten}</div>
                {phai.phan_thuong && (
                  <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 140 }}>
                    {phai.phan_thuong}
                  </div>
      ...
      <AnimatePresence>
        {selectedLienKet && <LienKetDetailModal item={selectedLienKet} onClose={() => setSelectedLienKet(null)} />}
      </AnimatePresence>
    </div>
  );
}
