...
// ── Highlight {text|color} — hỗ trợ: red, purple, yellow, blue, green + xuống dòng \n ──
function HighlightText({ text }: { text: string }) {
  // Split theo dòng trước, rồi xử lý highlight trong từng dòng
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}
              ...
              {items.map(item => (
                <button key={item.id} onClick={() => setSelected(p => p?.id === item.id ? null : item)}
                  ...
                  {/* Ảnh full, không crop */}
                  <div style={{ width: 128, background: 'var(--bg-sunken)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {item.image_url
                      ? <img src={item.image_url} alt={item.ten}
                  ...
                  {/* Tên bên dưới ảnh */}
                  <div style={{ fontSize: 18, fontWeight: 400, color: 'var(--text-1)',
