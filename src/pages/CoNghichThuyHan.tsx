...
// ── Highlight {text|color} — hỗ trợ: red, purple, yellow, blue, green ──
...
function TrangBiTab() {
  ...
  const [selected, setSelected] = useState<CoNghichTrangBiRow | null>(null);

  useEffect(() => {
    supabase.from('co_nghich_trang_bi').select('*').order('created_at', { ascending: true })
              ...
              {selected ? (
                <motion.div key={selected.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.18 }}>
                  ...
                  <div style={{ padding: '12px 16px' }}>
                    <div style={{ fontSize: 13.5, fontWeight: 700, color: 'var(--text-1)', marginBottom: 10 }}>Hiệu Quả</div>
...
function TamNgoTab() {
  ...
  const [selected, setSelected] = useState<CoNghichBuffRow | null>(null);

  useEffect(() => {
    supabase.from('co_nghich_buff').select('*').order('created_at', { ascending: true })
      .then(({ data }) => { setItems(data ?? []); setLoading(false); });
  }, []);
...
function QuanCoModal({ item, onClose }: { item: CoNghichQuanCoRow; onClose: () => void }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, y: 12 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 12 }}
        ...
        style={{ maxWidth: 760, maxHeight: '90vh', overflowY: 'auto', backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)', overflow: 'hidden', borderRadius: 16 }}
        onClick={e => e.stopPropagation()}>
        {/* Khu vực 1: Tên + xu + tags */}
        <div style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-sunken)', display: 'flex', alignItems: 'stretch', position: 'relative' }}>
          {item.image_url && (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 2 }}>
              ...
              {item.gia_xu && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <img src="/images/co-nghịch-thuy-han/icon-vang.png" alt="vàng" style={{ width: 20, height: 20, objectFit: 'contain' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
        ...
        <div style={{ padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
              {item.lien_ket_phe && item.lien_ket_phe.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-3)', flexShrink: 0 }}>Liên kết phe:</span>
                  {item.lien_ket_phe.map(slug => {
                    const info = LIEN_KET_PHE.find(p => p.slug === slug);
                    if (!info) return null;
                    return (
                      <div key={slug} title={info.ten} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 12px 4px 6px', borderRadius: 20, background: 'var(--bg-sunken)', border: '1px solid var(--border)' }}>
                        <img src={`/images/co-nghịch-thuy-han/lien-ket-phe/${slug}.png`} alt={info.ten} style={{ width: 32, height: 32, objectFit: 'contain' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-2)', whiteSpace: 'nowrap' }}>{info.ten}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              {item.lien_ket_phai && item.lien_ket_phai.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-3)', flexShrink: 0 }}>Liên kết phái:</span>
                  {item.lien_ket_phai.map(slug => {
                    const info = LIEN_KET_PHAI.find(p => p.slug === slug);
                    if (!info) return null;
                    return (
                      <div key={slug} title={info.ten} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 12px 4px 6px', borderRadius: 20, background: 'var(--bg-sunken)', border: '1px solid var(--border)' }}>
                        <img src={`/images/co-nghịch-thuy-han/lien-ket-phai/${slug}.png`} alt={info.ten} style={{ width: 32, height: 32, objectFit: 'contain' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-2)', whiteSpace: 'nowrap' }}>{info.ten}</span>
                      </div>
                    );
                  })}
          ...
          {item.ky_nang_ten && (
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
              <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--text-3)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 10 }}>Kỹ Năng</div>
              <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                {item.ky_nang_icon && (
                  <div style={{ width: 48, height: 48, flexShrink: 0, borderRadius: 6, overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--bg-sunken)' }}>
                    <img src={item.ky_nang_icon} alt={item.ky_nang_ten} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </div>
                )}
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-1)', marginBottom: 6, fontFamily: 'var(--font-skill)' }}>{item.ky_nang_ten}</div>
                  {item.ky_nang_mo_ta && <div style={{ fontSize: 13.5, color: 'var(--text-1)', lineHeight: 1.8 }}><HighlightText text={item.ky_nang_mo_ta} /></div>}
                </div>
              </div>
            </div>
          )}
          ...
          {item.chi_so && (
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
              <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--text-3)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 10 }}>Chỉ Số</div>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)' }}>
                    <th style={{ padding: '6px 10px', textAlign: 'left', color: 'var(--text-3)', fontWeight: 700, fontSize: 12 }}>Chỉ số</th>
                    {['★', '★★', '★★★'].map((s, i) => (
                      <th key={i} style={{ padding: '6px 14px', textAlign: 'center', color: '#d97706', fontWeight: 700 }}>{s}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {CHI_SO_ICONS.map(({ key, label, color }) => {
                    const vals = item.chi_so?.[key];
                    if (!vals || vals.length === 0) return null;
                    return (
                      <tr key={key} style={{ borderBottom: '1px solid var(--border)' }}>
                        <td style={{ padding: '7px 10px', fontWeight: 700, color }}>{label}</td>
                        {[0, 1, 2].map(idx => (
                          <td key={idx} style={{ padding: '7px 14px', textAlign: 'center', color: 'var(--text-1)' }}>{vals[idx] ?? '—'}</td>
                        ))}
                      </tr>
                    );
                  })}
...
function QuanCoTab() {
  ...
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<CoNghichQuanCoRow | null>(null);

  useEffect(() => {
    supabase.from('co_nghich_quan_co').select('*').order('so_sao', { ascending: false })
      .then(({ data }) => { setItems(data ?? []); setLoading(false); });
  }, []);
...
export default function CoNghichThuyHan() {
  const [mainTab, setMainTab] = useState<MainTab>('trang-bi');
