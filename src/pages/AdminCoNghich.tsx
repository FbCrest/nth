...
// ── Image uploader ──
function ImageUploader({ bucket, preview, onChange }: {
  bucket: string; preview: string; onChange: (url: string) => void;
}) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <label className="flex items-center gap-2 px-3 py-2 rounded-xl border cursor-pointer text-xs font-semibold justify-center"
        ...
        <Upload size={13} /> {uploading ? 'Đang upload...' : 'Chọn ảnh'}
...
function ModalTrangBi({ item, onClose, onSaved }: { item: CoNghichTrangBiRow | null; onClose: () => void; onSaved: () => void; }) {
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    const { error: dbErr } = isEdit
      ? await supabase.from('co_nghich_trang_bi').update(payload).eq('id', item!.id)
      : await supabase.from('co_nghich_trang_bi').insert(payload);
    setSaving(false);
...
function ModalBuff({ item, onClose, onSaved }: { item: CoNghichBuffRow | null; onClose: () => void; onSaved: () => void; }) {
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    const { error: dbErr } = isEdit
      ? await supabase.from('co_nghich_buff').update(payload).eq('id', item!.id)
      : await supabase.from('co_nghich_buff').insert(payload);
    setSaving(false);
    if (dbErr) { setError(dbErr.message); return; }
    onSaved(); onClose();
  };
...
function SectionTrangBi({ onAddNew }: { onAddNew: (trigger: () => void) => void }) {
  ...
  const fetch = async () => {
    setLoading(true);
    const { data } = await supabase.from('co_nghich_trang_bi').select('*').order('created_at', { ascending: false });
  ...
  const handleDelete = async (id: string) => {
    await supabase.from('co_nghich_trang_bi').delete().eq('id', id);
...
function SectionBuff({ onAddNew }: { onAddNew: (trigger: () => void) => void }) {
  ...
  const fetch = async () => {
    setLoading(true);
    const { data } = await supabase.from('co_nghich_buff').select('*').order('created_at', { ascending: false });
    setItems(data ?? []); setLoading(false);
  };
  useEffect(() => {
    fetch();
  ...
  const handleDelete = async (id: string) => {
    await supabase.from('co_nghich_buff').delete().eq('id', id);
    setDeleteId(null); fetch();
  };
...
function ModalQuanCo({ item, onClose, onSaved }: { item: CoNghichQuanCoRow | null; onClose: () => void; onSaved: () => void; }) {
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    const { error: dbErr } = isEdit
      ? await supabase.from('co_nghich_quan_co').update(payload).eq('id', item!.id)
      : await supabase.from('co_nghich_quan_co').insert(payload);
    setSaving(false);
    if (dbErr) { setError(dbErr.message); return; }
    onSaved(); onClose();
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.96, y: 8 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 8 }}
        className="rounded-2xl border shadow-2xl w-full" style={{ maxWidth: 900, maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}
        onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'var(--border)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ flex: 1, minHeight: 0, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          <div className="flex gap-5 p-5">
            ...
            <div style={{ width: 150, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label className="text-xs font-bold uppercase tracking-wider mb-1.5 block" style={{ color: 'var(--text-3)' }}>Ảnh tướng</label>
                <ImageUploader bucket="co-nghich-images" preview={form.image_url} onChange={url => setForm(p => ({ ...p, image_url: url }))} />
              </div>
              <div>
                <label className="text-xs font-bold uppercase tracking-wider mb-1.5 block" style={{ color: 'var(--text-3)' }}>Ảnh kỹ năng</label>
                <ImageUploader bucket="co-nghich-images" preview={form.ky_nang_icon_url} onChange={url => setForm(p => ({ ...p, ky_nang_icon_url: url }))} />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 flex-1 min-w-0">
                  ...
                  {LIEN_KET_PHE_LIST.map(p => {
                    ...
                    return (
                      <button key={p.slug} type="button" onClick={() => toggleSlug('lien_ket_phe', p.slug)} style={toggleBtn(active)}>
                        <img src={`/images/co-nghịch-thuy-han/lien-ket-phe/${p.slug}.png`} alt={p.ten} style={{ width: 18, height: 18, objectFit: 'contain' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                  ...
                  {LIEN_KET_PHAI_LIST.map(p => {
                    ...
                    return (
                      <button key={p.slug} type="button" onClick={() => toggleSlug('lien_ket_phai', p.slug)} style={toggleBtn(active)}>
                        <img src={`/images/co-nghịch-thuy-han/lien-ket-phai/${p.slug}.png`} alt={p.ten} style={{ width: 18, height: 18, objectFit: 'contain' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                        <span style={{ fontSize: 11, fontWeight: 700, color: active ? '#dc2626' : 'var(--text-2)' }}>{p.ten}</span>
                      </button>
                    );
                  })}
              ...
              <div><label className="text-xs font-bold uppercase tracking-wider mb-1 block" style={{ color: 'var(--text-3)' }}>Mô tả kỹ năng</label><textarea value={form.ky_nang_mo_ta} onChange={f('ky_nang_mo_ta')} rows={3} style={{ ...inputStyle, resize: 'vertical' }} /></div>
              <div><label className="text-xs font-bold uppercase tracking-wider mb-1 block" style={{ color: 'var(--text-3)' }}>Chỉ số (JSON)</label><textarea value={form.chi_so} onChange={f('chi_so')} rows={3} placeholder='{"khi_huyet":[1470,2646,14288],"tan_cong":[59,89,199]}' style={{ ...inputStyle, resize: 'vertical', fontFamily: 'monospace', fontSize: 12 }} /></div>
            </div>
          </div>
          {error && <p className="text-xs text-red-500 px-5 pb-2">{error}</p>}
          <div className="flex gap-2 px-5 pb-5">
...
function SectionQuanCo({ onAddNew }: { onAddNew: (trigger: () => void) => void }) {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data } = await supabase.from('co_nghich_quan_co').select('*').order('created_at', { ascending: false });
    setItems(data ?? []); setLoading(false);
  };
  useEffect(() => { fetchItems(); onAddNew(() => { setEditItem(null); setModalOpen(true); }); }, []);

  const handleDelete = async (id: string) => {
    await supabase.from('co_nghich_quan_co').delete().eq('id', id);
...
// ── Main Admin ──
type AdminTab = 'trang-bi' | 'tam-ngo' | 'quan-co';
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="p-4 sm:p-6 w-full">
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '2px solid var(--border)' }}>
        {(['quan-co', 'trang-bi', 'tam-ngo'] as AdminTab[]).map(tab => {
          const labels: Record<AdminTab, string> = { 'quan-co': 'Quân Cờ', 'trang-bi': 'Trang Bị', 'tam-ngo': 'Tâm Ngộ' };
