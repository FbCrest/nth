...
// Default public paths cho từng dirKey
...
// ── Local Image Uploader — dùng /api/upload-image (base64 qua server, không trigger HMR) ──
function LocalImageUploader({ preview, onChange, dirKey = 'default' }: {
  ...
  // localPreview: blob URL hiển thị ngay, độc lập với preview prop
  const [localPreview, setLocalPreview] = useState('');

  useEffect(() => {
    loadHandleData(dirKey).then(d => {
      console.log('[Uploader] loaded IndexedDB for dirKey:', dirKey, '→', d ? { name: d.handle.name, publicPath: d.publicPath } : null);
      if (d) setDirInfo({ name: d.handle.name, publicPath: d.publicPath });
    }).catch(() => {});
  }, [dirKey]);

  // Xóa entry trong IndexedDB cho dirKey này
  const clearHandle = async () => {
    await new Promise<void>((resolve, reject) => {
      ...
      req.onsuccess = () => {
        const tx = req.result.transaction(DB_STORE, 'readwrite');
        tx.objectStore(DB_STORE).delete(dirKey);
        tx.oncomplete = () => resolve();
        tx.onerror = () => reject(tx.error);
      };
      req.onerror = () => reject(req.error);
    ...
    setDirInfo(null);
    console.log('[Uploader] cleared IndexedDB for dirKey:', dirKey);
  };

  // Reset localPreview chỉ khi mở item MỚI (preview thay đổi từ bên ngoài),
  // KHÔNG reset khi chính ta vừa gọi onChange (lúc đó localPreview đang giữ blob URL cần hiển thị)
  const justUploadedRef = React.useRef(false);

  // Debug: log mỗi khi preview prop hoặc localPreview thay đổi
  useEffect(() => {
    console.log('[Uploader] preview prop changed:', preview);
    if (justUploadedRef.current) {
      // Vừa upload xong, onChange đã set preview — giữ nguyên localPreview (blob URL)
  ...
  useEffect(() => {
    console.log('[Uploader] localPreview changed:', localPreview);
  }, [localPreview]);

  const pickDir = async () => {
    try {
      const handle = await (window as any).showDirectoryPicker({ mode: 'readwrite' });
      // Luôn ghép path mới: base + tên folder vừa chọn
      const base = DIR_KEY_DEFAULTS[dirKey] ?? 'images/co-nghich-thuy-han';
      const defaultPath = base + '/' + handle.name;
      console.log('[pickDir] dirKey:', dirKey);
      console.log('[pickDir] handle.name:', handle.name);
      console.log('[pickDir] defaultPath set to pathInput:', defaultPath);
  ...
  const confirmDir = async () => {
    if (!pendingHandle || !pathInput.trim()) return;
    console.log('[confirmDir] saving dirKey:', dirKey, '→ handle:', pendingHandle.name, '→ publicPath:', pathInput.trim());
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('[Uploader] set localPreview (blob):', blobUrl);
    ...
    try {
      ...
      const publicPath = data?.publicPath ?? DIR_KEY_DEFAULTS[dirKey] ?? 'images/co-nghich-thuy-han';
      console.log('[Uploader] dirKey:', dirKey);
      console.log('[Uploader] IndexedDB data:', data ? { name: data.handle.name, publicPath: data.publicPath } : null);
      console.log('[Uploader] uploading to publicPath:', publicPath, '| fileName:', file.name);

      const res = await fetch('/api/upload-image', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ path: publicPath, fileName: file.name, base64 }),
      });
      const result = await res.json();
      console.log('[Uploader] upload result:', result);
      if (!result.ok) throw new Error(result.error ?? 'Upload thất bại');

      console.log('[Uploader] calling onChange with url:', result.url);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/* Confirm path sau khi pick folder */}
      {pendingHandle ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '6px 8px', borderRadius: 8, border: '1px solid rgba(239,68,68,0.4)', background: 'rgba(239,68,68,0.06)' }}>
          <span style={{ fontSize: 11, color: 'var(--text-3)' }}>
            Folder: <strong>{pendingHandle.name}</strong> — đường dẫn public:
          </span>
          <input
            value={pathInput}
            onChange={e => setPathInput(e.target.value)}
            placeholder={DIR_KEY_DEFAULTS[dirKey] ?? `images/co-nghich-thuy-han/.../${pendingHandle.name}`}
            style={{ ...inputStyle, fontSize: 11, padding: '4px 8px' }}
          />
          <div style={{ display: 'flex', gap: 4 }}>
            <button type="button" onClick={() => setPendingHandle(null)}
              style={{ flex: 1, padding: '3px 0', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg-sunken)', cursor: 'pointer', fontSize: 11, color: 'var(--text-2)' }}>
              Hủy
            </button>
            <button type="button" onClick={confirmDir} disabled={!pathInput.trim()}
              style={{ flex: 1, padding: '3px 0', borderRadius: 6, border: 'none', background: '#dc2626', cursor: 'pointer', fontSize: 11, fontWeight: 700, color: '#fff', opacity: pathInput.trim() ? 1 : 0.4 }}>
              Xác nhận
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <button type="button" onClick={pickDir}
            ...
            style={{ flex: 1, borderColor: dirInfo ? 'rgba(239,68,68,0.4)' : 'var(--border)', color: dirInfo ? '#dc2626' : 'var(--text-2)', backgroundColor: 'var(--bg-sunken)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            📁 {dirInfo ? dirInfo.name : 'Chọn thư mục'}
          </button>
          {dirInfo && (
            <button type="button" onClick={clearHandle} title="Xóa thư mục đã lưu"
              style={{ padding: '4px 8px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-sunken)', cursor: 'pointer', color: 'var(--text-3)', fontSize: 11, flexShrink: 0 }}>
              ✕
            </button>
          )}
        </div>
      ...
      <label
        className="flex items-center gap-2 px-3 py-2 rounded-xl border cursor-pointer text-xs font-semibold justify-center"
        style={{ borderColor: 'var(--border)', color: 'var(--text-2)', backgroundColor: 'var(--bg-sunken)', opacity: (!pendingHandle && !uploading) ? 1 : 0.4, pointerEvents: (pendingHandle || uploading) ? 'none' : 'auto', whiteSpace: 'nowrap' }}
      >
        <Upload size={13} /> {uploading ? 'Đang upload...' : 'Chọn ảnh & sao chép'}
        <input type="file" accept="image/*" className="hidden" onChange={handleFile} disabled={uploading || !!pendingHandle} />
      </label>
    </div>
  );
}
...
function ImageUploader({ preview, onChange, dirKey = 'default' }: { bucket?: string; preview: string; onChange: (url: string) => void; dirKey?: string; }) {
  return <LocalImageUploader dirKey={dirKey} preview={preview} onChange={(url) => onChange(url)} />;
}
...
function ModalTrangBi({ item, onClose, onSaved }: { item: CoNghichTrangBiRow | null; onClose: () => void; onSaved: () => void; }) {
  ...
  // Debug: theo dõi form.image_url
  useEffect(() => {
    console.log('[ModalTrangBi] form.image_url changed:', form.image_url);
  }, [form.image_url]);
...
function SectionTrangBi({ onAddNew }: { onAddNew: (trigger: () => void) => void }) {
  ...
  const [filterDanhMuc, setFilterDanhMuc] = useState<string>(() => {
    return localStorage.getItem('admin-trang-bi-filter') ?? 'Tất cả';
  });

  const handleFilterChange = (cat: string) => {
    setFilterDanhMuc(cat);
    localStorage.setItem('admin-trang-bi-filter', cat);
  };
  const dragId = React.useRef<string | null>(null);
  const [dragOverId, setDragOverId] = useState<string | null>(null);

  const DANH_MUC_FILTERS = ['Tất cả', 'Tấn công', 'Phòng thủ', 'Đặc biệt'];
  ...
  const filteredItems = filterDanhMuc === 'Tất cả' ? items : items.filter(i => i.danh_muc === filterDanhMuc);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        {DANH_MUC_FILTERS.map(cat => {
          const isActive = filterDanhMuc === cat;
          return (
            <button key={cat} onClick={() => handleFilterChange(cat)}
              style={{
                padding: '6px 16px', fontSize: 13, fontWeight: 700, borderRadius: 8, cursor: 'pointer',
                border: `1px solid ${isActive ? 'rgba(239,68,68,0.5)' : 'var(--border)'}`,
                color: isActive ? '#dc2626' : 'var(--text-2)',
                background: isActive ? 'rgba(239,68,68,0.08)' : 'var(--bg-sunken)',
...
type LienKetItem = {
  id: string; ten: string; ten_zh?: string | null; slug: string; image_url: string | null;
  phan_thuong: string | null;
  hieu_ung_lien_ket: string | null;
  quan_co_images: string[] | null;   // mảng URL ảnh quân cờ
};
...
function ModalLienKetItem({ item, type, onClose, onSaved }: {
  ...
  const [form, setForm] = useState({
    ten: item?.ten ?? '',
    ten_zh: (item as any)?.ten_zh ?? '',
    slug: item?.slug ?? '',
    image_url: item?.image_url ?? '',
    phan_thuong: item?.phan_thuong ?? '',
    hieu_ung_lien_ket: item?.hieu_ung_lien_ket ?? '',
    quan_co_images: (item?.quan_co_images ?? []) as string[],
  });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.ten.trim() || !form.slug.trim()) { setError('Tên và slug không được trống'); return; }
    setSaving(true); setError('');
    // Resolve blob URLs → public URLs trước khi lưu
    const resolvedImages = form.quan_co_images.map(url =>
      url.startsWith('blob:') ? (blobToPublicRef.current[url] ?? url) : url
    );
    const payload = {
      ten: form.ten.trim(),
      ...
      phan_thuong: form.phan_thuong.trim() || null,
      hieu_ung_lien_ket: form.hieu_ung_lien_ket.trim() || null,
      quan_co_images: resolvedImages.length > 0 ? resolvedImages : null,
    };
    try {
      const method = isEdit ? 'PUT' : 'POST';
      const body = isEdit ? { ...payload, id: item!.id } : payload;
  ...
  // Upload ảnh quân cờ — dùng cùng flow LocalImageUploader
  const quanCoDirKey = type === 'phe' ? 'lien-ket-quan-co-phe' : 'lien-ket-quan-co-phai';
  const [quanCoDirInfo, setQuanCoDirInfo] = useState<{ name: string; publicPath: string } | null>(null);
  const [quanCoPendingHandle, setQuanCoPendingHandle] = useState<FileSystemDirectoryHandle | null>(null);
  const [quanCoPathInput, setQuanCoPathInput] = useState('');
  const blobToPublicRef = React.useRef<Record<string, string>>({});

  useEffect(() => {
    loadHandleData(quanCoDirKey).then(d => {
      if (d) setQuanCoDirInfo({ name: d.handle.name, publicPath: d.publicPath });
    }).catch(() => {});
  }, [quanCoDirKey]);

  const pickQuanCoDir = async () => {
    try {
      const handle = await (window as any).showDirectoryPicker({ mode: 'readwrite' });
      // Pre-fill bằng tên folder — user tự sửa path cho đúng
      setQuanCoPendingHandle(handle);
      setQuanCoPathInput('images/co-nghich-thuy-han/' + handle.name);
    } catch { }
  };

  const confirmQuanCoDir = async () => {
    if (!quanCoPendingHandle || !quanCoPathInput.trim()) return;
    await saveHandle(quanCoDirKey, quanCoPendingHandle, quanCoPathInput.trim());
    setQuanCoDirInfo({ name: quanCoPendingHandle.name, publicPath: quanCoPathInput.trim() });
    setQuanCoPendingHandle(null);
  };

  const clearQuanCoDir = async () => {
    await new Promise<void>((resolve, reject) => {
      const req = indexedDB.open(DB_NAME, 1);
      req.onupgradeneeded = () => req.result.createObjectStore(DB_STORE);
      req.onsuccess = () => {
        const tx = req.result.transaction(DB_STORE, 'readwrite');
        tx.objectStore(DB_STORE).delete(quanCoDirKey);
        tx.oncomplete = () => resolve();
        tx.onerror = () => reject(tx.error);
      };
      req.onerror = () => reject(req.error);
    });
    setQuanCoDirInfo(null);
  };

  const handleAddQuanCoImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    // Preview ngay bằng blob URL — không đổi sau khi upload xong để tránh HMR reset
    const blobUrl = URL.createObjectURL(file);
    setForm(p => ({ ...p, quan_co_images: [...p.quan_co_images, blobUrl] }));
    const base64 = await new Promise<string>((resolve, reject) => {
      const reader = new FileReader();
    ...
    const publicPath = quanCoDirInfo?.publicPath ?? 'images/co-nghich-thuy-han/lien-ket-quan-co';
    const res = await fetch('/api/upload-image', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ path: publicPath, fileName: file.name, base64 }),
    });
    const result = await res.json();
    if (result.ok) {
      blobToPublicRef.current[blobUrl] = result.url;
    }
    e.target.value = '';
  };

  const removeQuanCoImage = (idx: number) => {
    setForm(p => ({ ...p, quan_co_images: p.quan_co_images.filter((_, i) => i !== idx) }));
  };

  const hieu_ungRef = React.useRef<HTMLTextAreaElement>(null);
  const mo_taRef = React.useRef<HTMLTextAreaElement>(null);

  const insertBulletAt = (
    ref: React.RefObject<HTMLTextAreaElement>,
    value: string,
    onChange: (v: string) => void
  ) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[200] flex items-center justify-center p-2" onClick={onClose}>
      <motion.div initial={{ scale: 0.96, y: 8 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 8 }}
        className="rounded-2xl border shadow-2xl w-full" style={{ maxWidth: 1100, maxHeight: '94vh', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}
        onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden' }}>
            ...
            {/* CỘT TRÁI — ảnh + fields cơ bản */}
            <div style={{ width: 280, flexShrink: 0, borderRight: '1px solid var(--border)', padding: '16px 20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
              {/* Ảnh uploader */}
              <div style={{ width: 160 }}>
                <LocalImageUploader
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-wider mb-1 block" style={{ color: 'var(--text-3)' }}>Tên chữ Hán</label>
                <input value={form.ten_zh} onChange={e => setForm(p => ({ ...p, ten_zh: e.target.value }))} placeholder="白帝城" style={inputStyle} />
              </div>
              <div>
                ...
                <p style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 3 }}>Map với file ảnh (bach-de-thanh.png)</p>
              </div>
            </div>

            {/* CỘT PHẢI — mô tả + hiệu ứng + quân cờ */}
            <div style={{ flex: 1, minWidth: 0, padding: '16px 20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
              {/* Mô tả */}
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                  <label className="text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--text-3)' }}>Mô tả</label>
                  <button type="button" onClick={() => insertBulletAt(mo_taRef, form.phan_thuong, v => setForm(p => ({ ...p, phan_thuong: v })))}
                    style={{ fontSize: 12, padding: '1px 8px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg-sunken)', cursor: 'pointer', color: 'var(--text-2)', fontWeight: 700 }}>+ •</button>
                </div>
                <textarea ref={mo_taRef} value={form.phan_thuong} onChange={e => setForm(p => ({ ...p, phan_thuong: e.target.value }))}
                  rows={3} placeholder="Quân cờ thuộc liên kết này nhận hiệu ứng..."
                  style={{ ...inputStyle, resize: 'vertical', fontSize: 13 }} />
                <p style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 3 }}>Hiển thị bên dưới tên trong modal</p>
              </div>
              {/* Hiệu ứng liên kết */}
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                  <label className="text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--text-3)' }}>Hiệu Ứng Liên Kết</label>
                  <button type="button" onClick={() => insertBulletAt(hieu_ungRef, form.hieu_ung_lien_ket, v => setForm(p => ({ ...p, hieu_ung_lien_ket: v })))}
                    style={{ fontSize: 12, padding: '1px 8px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg-sunken)', cursor: 'pointer', color: 'var(--text-2)', fontWeight: 700 }}>+ •</button>
                </div>
                <p style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 6 }}>
                  Mỗi dòng: <code style={{ background: 'var(--bg-sunken)', padding: '1px 5px', borderRadius: 4 }}>2 Nội dung hiệu ứng mốc 2</code>
                </p>
                <textarea ref={hieu_ungRef}
                  value={form.hieu_ung_lien_ket}
                  onChange={e => setForm(p => ({ ...p, hieu_ung_lien_ket: e.target.value }))}
                  rows={7}
                  placeholder={"2 Nhận 1 bát Mạnh Bà Thang\n4 Mỗi lần giết 1 quân cờ đồng đội nhận 4/12 điểm\n6 Tăng 50% hiệu quả nhận điểm\n8 Hiệu quả Mạnh Bà Thang tăng 100%"}
                  style={{ ...inputStyle, resize: 'vertical', fontFamily: 'monospace', fontSize: 13, flex: 1 }}
                />
              </div>

              {/* Danh sách quân cờ */}
              <div>
                <label className="text-xs font-bold uppercase tracking-wider mb-2 block" style={{ color: 'var(--text-3)' }}>Danh Sách Quân Cờ</label>
                {/* Folder picker */}
                {quanCoPendingHandle ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '6px 8px', borderRadius: 8, border: '1px solid rgba(239,68,68,0.4)', background: 'rgba(239,68,68,0.06)', marginBottom: 8 }}>
                    <span style={{ fontSize: 11, color: 'var(--text-3)' }}>Folder: <strong>{quanCoPendingHandle.name}</strong> — đường dẫn public:</span>
                    <input value={quanCoPathInput} onChange={e => setQuanCoPathInput(e.target.value)}
                      placeholder="images/co-nghich-thuy-han/..."
                      style={{ ...inputStyle, fontSize: 11, padding: '4px 8px' }} />
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button type="button" onClick={() => setQuanCoPendingHandle(null)}
                        style={{ flex: 1, padding: '3px 0', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg-sunken)', cursor: 'pointer', fontSize: 11, color: 'var(--text-2)' }}>Hủy</button>
                      <button type="button" onClick={confirmQuanCoDir} disabled={!quanCoPathInput.trim()}
                        style={{ flex: 1, padding: '3px 0', borderRadius: 6, border: 'none', background: '#dc2626', cursor: 'pointer', fontSize: 11, fontWeight: 700, color: '#fff', opacity: quanCoPathInput.trim() ? 1 : 0.4 }}>Xác nhận</button>
                    </div>
                  </div>
                ) : (
                  <div style={{ display: 'flex', gap: 4, marginBottom: 8 }}>
                    <button type="button" onClick={pickQuanCoDir}
                      className="flex items-center gap-2 px-3 py-2 rounded-xl border cursor-pointer text-xs font-semibold"
                      style={{ flex: 1, borderColor: quanCoDirInfo ? 'rgba(239,68,68,0.4)' : 'var(--border)', color: quanCoDirInfo ? '#dc2626' : 'var(--text-2)', backgroundColor: 'var(--bg-sunken)' }}>
                      📁 {quanCoDirInfo ? quanCoDirInfo.name : 'Chọn thư mục'}
                    </button>
                    {quanCoDirInfo && (
                      <button type="button" onClick={clearQuanCoDir} title="Xóa thư mục đã lưu"
                        style={{ padding: '4px 8px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-sunken)', cursor: 'pointer', color: 'var(--text-3)', fontSize: 11 }}>✕</button>
                    )}
                  </div>
                )}
                {/* Grid ảnh */}
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'flex-start' }}>
                  {form.quan_co_images.map((url, idx) => (
                    <div key={idx} style={{ position: 'relative', width: 64, flexShrink: 0 }}>
                      <img src={url} alt="" style={{ width: 64, height: 72, objectFit: 'cover', display: 'block', border: '1px solid var(--border)' }} />
                      <button type="button" onClick={() => removeQuanCoImage(idx)}
                        style={{ position: 'absolute', top: -5, right: -5, width: 16, height: 16, borderRadius: '50%', background: '#dc2626', border: 'none', cursor: 'pointer', color: '#fff', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>×</button>
                    </div>
                  ))}
                  <label style={{
                    width: 64, height: 72, flexShrink: 0, cursor: quanCoDirInfo && !quanCoPendingHandle ? 'pointer' : 'not-allowed',
                    border: '2px dashed var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                    color: 'var(--text-3)', fontSize: 20, opacity: quanCoDirInfo && !quanCoPendingHandle ? 1 : 0.4,
                  }}
                    onMouseEnter={e => { if (quanCoDirInfo) (e.currentTarget as HTMLElement).style.borderColor = '#dc2626'; }}
                    onMouseLeave={e => (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)'}>
                    +
                    <input type="file" accept="image/*" className="hidden" onChange={handleAddQuanCoImage} disabled={!quanCoDirInfo || !!quanCoPendingHandle} />
                  </label>
                </div>
              </div>
            </div>
          ...
          {/* Footer */}
          {error && <p className="text-xs text-red-500 px-5 py-2 border-t" style={{ borderColor: 'var(--border)' }}>{error}</p>}
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id} style={{ borderBottom: '1px solid var(--border)' }}
                    ...
                    <td className="px-4 py-3" style={{ borderRight: '1px solid var(--border)' }}>
                      <span style={{ fontSize: 12, fontFamily: 'monospace', color: 'var(--text-3)', background: 'var(--bg-sunken)', padding: '2px 6px', borderRadius: 4, whiteSpace: 'nowrap' }}>{item.slug}</span>
                    </td>
...
export default function AdminCoNghich() {
  ...
  const [activeTab, setActiveTab] = useState<AdminTab>(() => {
    return (localStorage.getItem('admin-co-nghich-tab') as AdminTab) ?? 'trang-bi';
  });
  const addNewRef = React.useRef<(() => void) | null>(null);

  const handleTabChange = (tab: AdminTab) => {
    setActiveTab(tab);
    localStorage.setItem('admin-co-nghich-tab', tab);
