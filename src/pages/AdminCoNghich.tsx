...
const LOAI_COLOR_MAP: Record<string, { color: string; bg: string; border: string }> = {
...
const DEFAULT_LOAI_COLOR = { color: '#7c3aed', bg: 'rgba(124,58,237,0.10)', border: 'rgba(124,58,237,0.35)' };
function getLoaiStyle(loai: string) { return LOAI_COLOR_MAP[loai] ?? DEFAULT_LOAI_COLOR; }
                          ...
                          {item.lien_ket_phe && item.lien_ket_phe.length > 0 && (
                            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                              {item.lien_ket_phe.map(slug => {
                                const info = LIEN_KET_PHE_LIST.find(p => p.slug === slug);
                                return (
                                  <div key={slug} style={{ display: 'flex', alignItems: 'center', gap: 3, padding: '2px 7px 2px 3px', borderRadius: 20, background: 'var(--bg-sunken)', border: '1px solid var(--border)' }}>
                                    <img src={`/images/co-nghich-thuy-han/lien-ket-phe/${slug}.png`} alt={slug}
                          ...
                          {item.lien_ket_phai && item.lien_ket_phai.length > 0 && (
                            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                              {item.lien_ket_phai.map(slug => {
                                const info = LIEN_KET_PHAI_LIST.find(p => p.slug === slug);
                                return (
                                  <div key={slug} style={{ display: 'flex', alignItems: 'center', gap: 3, padding: '2px 7px 2px 3px', borderRadius: 20, background: 'var(--bg-sunken)', border: '1px solid var(--border)' }}>
                                    <img src={`/images/co-nghich-thuy-han/lien-ket-phai/${slug}.png`} alt={slug}
                                      style={{ width: 22, height: 22, objectFit: 'contain', flexShrink: 0 }}
                                      onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                                    <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-2)', whiteSpace: 'nowrap' }}>{info?.ten ?? slug}</span>
                                  </div>
...
// ── Main Admin ──
type AdminTab = 'trang-bi' | 'tam-ngo' | 'quan-co' | 'buff-kiem' | 'icons';
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="p-4 sm:p-6 w-full">
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '2px solid var(--border)' }}>
        {(['quan-co', 'trang-bi', 'tam-ngo', 'buff-kiem', 'icons'] as AdminTab[]).map(tab => {
          const labels: Record<AdminTab, string> = { 'quan-co': 'Quân Cờ', 'trang-bi': 'Trang Bị', 'tam-ngo': 'Tâm Ngộ', 'buff-kiem': 'Buff Kiếm', 'icons': 'Icons' };
